...
async function fetchAll(path:string,fields:string){
  ...
  while(page<=200){
    ...
    try{j=JSON.parse(text)}catch{
      throw new Error(`Accurate mengembalikan response non-API. HTTP ${res.status}`);
    }
...
export async function syncAccurateMasters(kinds:Kind[]=['items','warehouses','accounts','branches']){
  ...
  try{
    if(kinds.includes('items')){
      ...
      await c.query('BEGIN'); await c.query('DELETE FROM items_cache');
    ...
    if(kinds.includes('warehouses')){
      ...
      await c.query('BEGIN'); await c.query('DELETE FROM warehouses_cache');
    ...
    if(kinds.includes('accounts')){
      ...
      await c.query('BEGIN'); await c.query('DELETE FROM accounts_cache');
    ...
    if(kinds.includes('branches')){
      const rows=await fetchAll('/api/branch/list.do','id,name');
      await c.query('BEGIN'); await c.query('DELETE FROM branches_cache');
      for(const x of rows){ if(!x?.name) continue; await c.query(`INSERT INTO branches_cache(accurate_id,name,updated_at) VALUES($1,$2,NOW()) ON CONFLICT(accurate_id) DO UPDATE SET name=EXCLUDED.name,updated_at=NOW()`,[x.id||null,x.name]); }
      await c.query('COMMIT'); result.branches=rows.length;
    }
    return result;
