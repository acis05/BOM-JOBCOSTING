...
async function fetchAll(path:string,fields:string){
  ...
  while(page<=200){
    ...
    const text=await res.text(); let j:any; try{j=JSON.parse(text)}catch{throw new Error(`Response Accurate tidak valid: ${text.slice(0,160)}`)}
    ...
    if(rows.length<pageSize) break; page++;
  }
  return out;
}
